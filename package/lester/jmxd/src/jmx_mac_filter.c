...
static int find_mac_filter_rule_index_by_id(struct uci_context *uci_ctx, int id) {
	int i;
    char id_str_uci[32];
    int num = jmx_uci_get_list_num(uci_ctx, "macfilter", "rule");
    for (i = 0; i < num; i++) {
        char buf[128];
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].id", i);
        if (jmx_uci_get_value(uci_ctx, buf, id_str_uci, sizeof(id_str_uci)) != 0) {
            continue;
        }
...
struct json_object *jmx_api_get_mac_filter_rules(struct json_object *req_obj) {
    ...
    for (i = 0; i < num; i++) {
        ...
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].id", i);
        if (jmx_uci_get_value(uci_ctx, buf, id_str, sizeof(id_str)) != 0) {
            LOG_ERROR("Failed to get id for rule[%d], skipping\n", i);
            continue; 
        }
        
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].name", i);
        jmx_uci_get_value(uci_ctx, buf, name_str, sizeof(name_str));
        
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].mode", i);
        if (jmx_uci_get_value(uci_ctx, buf, mode_str, sizeof(mode_str)) != 0) {
            strcpy(mode_str, "1");
        }
        
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].user_mac", i);
        jmx_uci_get_value(uci_ctx, buf, user_mac_str, sizeof(user_mac_str));
        
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].user_name", i);
        jmx_uci_get_value(uci_ctx, buf, user_name_str, sizeof(user_name_str)); 
        
        snprintf(buf, sizeof(buf), "macfilter.@rule[%d].enabled", i);
        if (jmx_uci_get_value(uci_ctx, buf, enabled_str, sizeof(enabled_str)) != 0) {
        ...
        if (jmx_uci_get_list_value(uci_ctx, buf, time_rule_list_buf, sizeof(time_rule_list_buf), " ") == 0) {
...
struct json_object *jmx_api_add_mac_filter_rule(struct json_object *req_obj) {
    ...
    jmx_uci_set_value(uci_ctx, buf, enabled_str);
    ...
    for (i = 0; i < time_rules_len; i++) {
        ...
        jmx_uci_add_list(uci_ctx, buf, time_rule_str);
    }
    
    jmx_uci_commit(uci_ctx, "macfilter");
    uci_free_context(uci_ctx);
...
struct json_object *jmx_api_get_mac_filter_whitelist(struct json_object *req_obj) {
    ...
    char mac_str[128] = {0};
    int num = jmx_uci_get_list_num(uci_ctx, "macfilter_whitelist", "whitelist_mac");
    for (i = 0; i < num; i++) {
        jmx_uci_get_array_value(uci_ctx, "macfilter_whitelist.@whitelist_mac[%d].mac", i, mac_str, sizeof(mac_str));
...
struct json_object *jmx_api_del_mac_filter_whitelist(struct json_object *req_obj) {
	int i;
    LOG_DEBUG("jmx_api_del_mac_filter_whitelist\n");
    ...
    jmx_uci_commit(uci_ctx, "macfilter_whitelist");
    uci_free_context(uci_ctx);
...
struct json_object *jmx_api_add_mac_filter_whitelist(struct json_object *req_obj) {
	int i, j;
    LOG_DEBUG("jmx_api_add_mac_filter_whitelist\n");
    ...
    for (i = 0; i < mac_list_len; i++) {
        ...
        int exists = 0;
        for (j = 0; j < num; j++) {
            jmx_uci_get_array_value(uci_ctx, "macfilter_whitelist.@whitelist_mac[%d].mac", j, mac_str, sizeof(mac_str));
        ...
        if (!exists) {
            jmx_uci_add_section(uci_ctx, "macfilter_whitelist", "whitelist_mac");
            char buf[128];
            snprintf(buf, sizeof(buf), "macfilter_whitelist.@whitelist_mac[-1].mac");
            jmx_uci_set_value(uci_ctx, buf, mac);
...
struct json_object *jmx_api_get_mac_filter_adv(struct json_object *req_obj) {
    ...
    int enable = jmx_network_control_macfilter_enabled();
    json_object_object_add(data_obj, "enable", json_object_new_int(enable));
    json_object_object_add(data_obj, "source", json_object_new_string("config.db"));
    return jmx_gen_api_response_data(API_CODE_SUCCESS, data_obj);
}
...
struct json_object *jmx_api_set_mac_filter_adv(struct json_object *req_obj) {
    ...
    int enable_value = json_object_get_boolean(enable_obj) ? 1 : 0;
    if (jmx_network_control_set_filter_enabled("mac", enable_value) != 0) {
        LOG_ERROR("Failed to persist macfilter enable to config.db\n");
        return jmx_gen_api_response_data(API_CODE_ERROR, NULL);
    }
