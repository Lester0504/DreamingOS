...
void jmx_init_time_status(jmx_run_time_status_t *status){
    status->filter = 0;
    status->deny_time = 0;
    status->allow_time = 0;
    status->match_time = 0;
...
int jmx_check_time_manual(jmx_time_config_t *t_config, jmx_run_time_status_t *status) {
	int i;
...
int jmx_check_time_dynamic(jmx_time_config_t *t_config, jmx_run_time_status_t *status) {
    time_t now = time(NULL);
    struct tm *current_time = localtime(&now);
    int current_minutes = current_time->tm_hour * 60 + current_time->tm_min;
...
int jmx_check_time(jmx_time_config_t *t_config, jmx_run_time_status_t *status) {
    time_t now = time(NULL);
    struct tm *current_time = localtime(&now);
    int current_wday = current_time->tm_wday; // 0=Sunday, 1=Monday, ..., 6=Saturday
    ...
    if (t_config->time_mode == 0) {
        ...
        return jmx_check_time_manual(t_config, status);
    } else {
        ...
        return jmx_check_time_dynamic(t_config, status);
    }
}


struct json_object * jmx_gen_api_response_data(int code, struct json_object *data_obj){
