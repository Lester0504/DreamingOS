...
enum jmx_nl_action {
	...
	JMX_NL_ACT_RULE_STATUS   = 14, /* kernel commit status reply */
};
...
/* Kernel reply to JMX_NL_ACT_RULE_VERSION. status is errno-style; the active
 * snapshot lets userspace prove the requested generation is actually live. */
struct jmx_nl_rule_status_msg {
	int32_t  action;          /* JMX_NL_ACT_RULE_STATUS */
	uint32_t request_action;  /* JMX_NL_ACT_RULE_VERSION */
	uint32_t version;         /* requested generation */
	int32_t  status;          /* 0 or negative errno */
	uint32_t active_version;  /* active generation after commit */
	uint32_t active_rules;    /* active v2 rule count */
} __packed;
