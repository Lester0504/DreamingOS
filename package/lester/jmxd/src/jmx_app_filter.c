...
jmx_run_time_status_t g_af_status;
...
static int find_rule_index_by_id(struct uci_context *uci_ctx, int id) {
    char id_str_uci[32];
	int i;
    int num = jmx_uci_get_list_num(uci_ctx, "appfilter", "rule");
    for (i = 0; i < num; i++) {
        char buf[128];
...
struct json_object *jmx_api_add_filter_rule(struct json_object *req_obj) {
    ...
    jmx_uci_add_section(uci_ctx, "appfilter", "rule");
    ...
    for (i = 0; i < app_ids_len; i++) {
        ...
        if (app_id > 0) {
            ...
            jmx_uci_add_list(uci_ctx, buf, app_id_str);
        }
    }
    
    jmx_uci_commit(uci_ctx, "appfilter");
    uci_free_context(uci_ctx);
...
struct json_object *jmx_api_get_appfilter_whitelist(struct json_object *req_obj){
    ...
    char mac_str[128] = {0};
    int num = jmx_uci_get_list_num(uci_ctx, "appfilter_whitelist", "whitelist_mac");
    for (i = 0; i < num; i++) {
        jmx_uci_get_array_value(uci_ctx, "appfilter_whitelist.@whitelist_mac[%d].mac", i, mac_str, sizeof(mac_str));
...
struct json_object *jmx_api_del_appfilter_whitelist(struct json_object *req_obj){
	int i;
    LOG_DEBUG("jmx_api_del_appfilter_whitelist\n");
    ...
    jmx_uci_commit(uci_ctx, "appfilter_whitelist");
    uci_free_context(uci_ctx);
...
struct json_object *jmx_api_add_appfilter_whitelist(struct json_object *req_obj){
	int i;
    LOG_DEBUG("jmx_api_add_appfilter_whitelist\n");
    ...
    for (i = 0; i < len; i++) {
        ...
        jmx_uci_add_section(uci_ctx, "appfilter_whitelist", "whitelist_mac");
        jmx_uci_set_value(uci_ctx, "appfilter_whitelist.@whitelist_mac[-1].mac", json_object_get_string(mac_obj));
    }
    jmx_uci_commit(uci_ctx, "appfilter_whitelist");
    uci_free_context(uci_ctx);
...
struct json_object *jmx_api_get_app_filter_adv(struct json_object *req_obj) {
    ...
    int enable = jmx_network_control_appfilter_enabled();
...
struct json_object *jmx_api_set_app_filter_adv(struct json_object *req_obj) {
    ...
    if (jmx_network_control_set_filter_enabled("app", enable_value) != 0) {
        LOG_ERROR("Failed to persist appfilter enable to config.db\n");
        return jmx_gen_api_response_data(API_CODE_ERROR, NULL);
    }
