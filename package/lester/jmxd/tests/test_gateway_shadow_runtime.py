...
PAIRING_SOURCE = ROOT / "src/jmx_gateway_shadow_pairing.c"
RUNTIME_SOURCE = ROOT / "src/jmx_gateway_shadow_runtime.c"
HEADER = ROOT / "src/jmx_gateway_shadow.h"
JSON_PREFIXES = tuple(
    Path(value)
    for value in (
        os.environ.get("JSON_C_PREFIX"),
...
JSON_PREFIX = next(
    (prefix for prefix in JSON_PREFIXES if (prefix / "include/json-c/json.h").is_file()),
    None,
)
OPENSSL_PREFIXES = tuple(
    Path(value)
    for value in (
...
OPENSSL_PREFIX = next(
    (prefix for prefix in OPENSSL_PREFIXES if (prefix / "include/openssl/evp.h").is_file()),
    None,
)
...
int main(void)
{
    ...
    json_object_object_add(cfg, "management_ipv4", json_object_new_string("192.0.2.2/24"));
    ...
    json_object_object_add(cfg, "heartbeat_prefix_length", json_object_new_int(30));
    ...
    json_object_object_add(cfg, "auth_key",
                           json_object_new_string("gateway-shadow-runtime-secret-2026"));
    ...
    assert(boolean(data, "secret_configured"));
    assert(!strstr(json_object_to_json_string(response),
                   "gateway-shadow-runtime-secret-2026"));
    ...
    assert(!strcmp(string(data, "error"), "gateway_shadow_disabled"));
    json_object_put(response);
...
def test_gateway_shadow_runtime_state_machine() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gateway-shadow-runtime-") as raw:
        ...
        if OPENSSL_PREFIX is not None:
            command += ["-I", str(OPENSSL_PREFIX / "include")]
        command += [
            str(SOURCE),
            str(PAIRING_SOURCE),
            str(RUNTIME_SOURCE),
        ...
        if JSON_PREFIX is not None and (JSON_PREFIX / "lib/libjson-c.a").is_file():
            command += [str(JSON_PREFIX / "lib/libjson-c.a")]
        else:
            command += ["-ljson-c"]
        if OPENSSL_PREFIX is not None and (OPENSSL_PREFIX / "lib/libcrypto.dylib").is_file():
            command += [str(OPENSSL_PREFIX / "lib/libcrypto.dylib")]
        else:
            command += ["-lcrypto"]
        ...
        secret_directory = directory / "gateway-shadow"
        environment["DREAMINGWRT_GATEWAY_SHADOW_DIR"] = str(secret_directory)
        subprocess.run([str(executable)], env=environment, check=True)
        secret_path = secret_directory / "auth.key"
        assert secret_path.is_file()
        assert secret_path.stat().st_mode & 0o777 == 0o600
        assert secret_path.read_text(encoding="utf-8").strip() == (
            "gateway-shadow-runtime-secret-2026"
        )
        assert b"gateway-shadow-runtime-secret-2026" not in database.read_bytes()
