export function mount(context = {}) {
  ...
  function tableMarkup(title, subtitle, headings, rows, empty = '暂无内容', options = {}) {
    return `<section class="user-auth-main-surface user-auth-table-card dwrt-kit-table-wrap dwrt-kit-datatable-wrap dwrt-kit-glass-surface" data-user-auth-list><div class="dwrt-kit-table-toolbar"><div class="dwrt-kit-table-title"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(subtitle)}</span></div><span class="dwrt-kit-table-count">${rows.length} 条</span></div><div class="dwrt-kit-table-scroll"><table class="dwrt-kit-table dwrt-kit-datatable user-auth-table"><thead><tr>${options.select ? '<th><input type="checkbox" data-user-auth-select-all aria-label="全选"></th>' : ''}${headings.map((heading) => `<th>${escapeHtml(heading)}</th>`).join('')}</tr></thead><tbody>${state.loading && !state.loaded ? `<tr><td colspan="${headings.length + (options.select ? 1 : 0)}" class="dwrt-kit-table-empty">正在读取用户认证数据</td></tr>` : rows.length ? rows.join('') : `<tr><td colspan="${headings.length + (options.select ? 1 : 0)}" class="dwrt-kit-table-empty">${escapeHtml(empty)}</td></tr>`}</tbody></table></div></section>`;
  }
  ...
  function editorFields() {
    ...
    if (state.drawer === 'access') return `<div class="user-auth-form-grid">${field('规则范围', 'scope', editor.scope, { tag: 'select', options: [['pre_authorization','认证前放行'],['post_authorization','认证后限制'],['restricted_dns','受限 DNS']] })}${field('地址类型', 'type', editor.type, { tag: 'select', options: [['cidr','IP / 网段'],['domain','域名'],['dns','DNS 服务器']] })}${field('地址 / 域名', 'value', editor.value, { wide: true, required: true, placeholder: '192.168.1.0/24 或 example.com' })}</div>`;
