const VERSION = '20260724-upnp-service-02';
...
export function mount(context = {}) {
  ...
  function switchControl(field, checked, options = {}) {
    ...
    return `<label class="upnp-switch ${disabled ? 'is-disabled' : ''}" data-dwrt-component="switch"><input type="checkbox" ${attribute}="${escapeHtml(attributeValue)}" aria-label="${escapeHtml(options.label || field)}" ${checked ? 'checked' : ''} ${disabled ? 'disabled' : ''}></label>`;
  }
  ...
  function settingGroup(id, title, summary, count, body, iconName) {
    const expanded = state.expanded === id;
    return `<section class="upnp-setting-group ${expanded ? 'is-expanded' : ''}" data-upnp-group="${id}"><button class="upnp-setting-group-toggle" type="button" data-upnp-group-toggle="${id}" aria-expanded="${expanded ? 'true' : 'false'}" aria-controls="upnp-group-${id}"><span class="upnp-setting-group-icon">${icon(iconName, 20)}</span><span><strong>${escapeHtml(title)}</strong><small>${escapeHtml(summary)}</small></span><em>${escapeHtml(count)}</em>${icon('chevronDown', 18)}</button><div class="upnp-setting-group-body" id="upnp-group-${id}" ${expanded ? '' : 'hidden'}>${body}</div></section>`;
  }

  function settingsMarkup() {
    const writable = cap('service_update');
    const protocolBody = `<div class="upnp-control-grid">
      ${switchRow('enabled', 'UPnP IGD', '允许受信任终端动态申请端口映射', { disabled: !writable })}
    ...
    return `<section class="upnp-settings-surface dwrt-kit-glass-surface" data-dwrt-component="surface">
      ...
      <div class="upnp-runtime-summary" aria-label="UPnP 运行状态"><span><small>当前映射</small><strong>${Math.max(firstNumber(stats.active_mappings), state.dynamicMappings.length)}</strong></span><span><small>今日请求</small><strong>${firstNumber(stats.requests_today).toLocaleString()}</strong></span><span><small>今日拒绝</small><strong>${firstNumber(stats.denied_today).toLocaleString()}</strong></span><span><small>安全模式</small><strong>${state.draft.secure_mode ? '已启用' : '已关闭'}</strong></span></div>
      <div class="upnp-setting-groups">
        ${settingGroup('protocol', '协议与安全', '控制端口映射协议与跨主机保护', '6 项', protocolBody, 'shieldCheck')}
        ${settingGroup('boundary', '网络边界与端口范围', '指定外网、内网、端口边界与链路带宽', '6 项', boundaryBody, 'network')}
        ${settingGroup('runtime', '运行参数', '管理日志、服务时间、通知、清理与 STUN 参数', '6 项', runtimeBody, 'clock')}
      </div>
    </section>${dirtyBarMarkup()}`;
  }

  function tableShell(title, meta, headings, rows, empty, count = rows.length) {
    return `<section class="dwrt-kit-table-wrap dwrt-kit-datatable-wrap dwrt-kit-glass-surface upnp-table-card" data-dwrt-component="data-table"><div class="dwrt-kit-table-toolbar"><div class="dwrt-kit-table-title"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(meta)}</span></div><span class="dwrt-kit-table-count">${count} 条</span></div><div class="dwrt-kit-table-scroll"><table class="dwrt-kit-table dwrt-kit-datatable upnp-table"><thead><tr>${headings.map((heading) => `<th>${escapeHtml(heading)}</th>`).join('')}</tr></thead><tbody data-upnp-table-body>${state.loading ? `<tr><td colspan="${headings.length}" class="dwrt-kit-table-empty">正在读取配置</td></tr>` : rows.length ? rows.join('') : `<tr><td colspan="${headings.length}" class="dwrt-kit-table-empty">${escapeHtml(empty)}</td></tr>`}</tbody></table></div></section>`;
  }
  ...
  function render() {
    ...
    root.innerHTML = `<section class="upnp-service-shell">${toolbarMarkup()}<main class="upnp-service-workbench">${noticeMarkup()}${renderContent()}</main>${aclDrawer()}${mappingDrawer()}${deleteConfirmationMarkup()}</section>`;
    ui.mountAll?.(root);
  }
