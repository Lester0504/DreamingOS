export function mount(context = {}) {
  ...
  function renderTable() {
    ...
    return `<section class="routing-table-card dwrt-kit-table-wrap dwrt-kit-datatable-wrap dwrt-kit-glass-surface">
      ...
      <div class="dwrt-kit-table-scroll routing-table-scroll" data-routing-scroll><table class="dwrt-kit-table dwrt-kit-datatable routing-table"><thead><tr>${columns.map((column) => `<th style="width:${column.width}px;min-width:${column.width}px"><button type="button" data-route-sort="${column.key}" ${column.key === 'actions' ? 'disabled' : ''}>${escapeHtml(column.label)}${sortIndicator(column.key)}</button></th>`).join('')}</tr></thead><tbody>${state.loading ? `<tr><td colspan="${columns.length}" class="dwrt-kit-table-empty">正在读取路由</td></tr>` : rows.length ? rows.map((row) => `<tr class="${row.enabled ? '' : 'is-disabled'}" data-route-id="${escapeHtml(row.id)}">${columns.map((column) => `<td class="is-${column.key}">${cellMarkup(row, column.key)}</td>`).join('')}</tr>`).join('') : `<tr><td colspan="${columns.length}" class="dwrt-kit-table-empty">${escapeHtml(state.error || '没有匹配的路由')}</td></tr>`}</tbody></table></div>
    </section>`;
  }
