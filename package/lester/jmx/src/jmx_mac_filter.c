...
int jmx_mac_filter_init(void) {
	...
	g_mac_rule_count = 0;
	jmx_mac_config_init(&g_mac_filter_whitelist);
...
void jmx_mac_filter_exit(void) {
	...
	g_mac_rule_count = 0;
	jmx_flush_mac_list(&g_mac_filter_whitelist);
	mac_filter_write_unlock();
}
...
static int jmx_add_mac_filter_rule(int rule_id) {
	...
	if (jmx_find_mac_filter_rule(rule_id)) {
		return -1;
	}
...
static int jmx_del_mac_from_rule(int rule_id, const unsigned char *mac) {
	...
	rule = jmx_find_mac_filter_rule(rule_id);
	if (rule) {
		node = jmx_find_mac_node(&rule->mac_list, mac);
		if (node) {
...
int jmx_api_add_mac_filter_rule(cJSON *data_obj) {
	...
	if (jmx_add_mac_filter_rule(rule_id_obj->valueint) < 0) {
		return -1;
	}
...
int jmx_api_mod_mac_filter_rule(cJSON *data_obj) {
	...
	rule = jmx_find_mac_filter_rule(rule_id_obj->valueint);
	if (!rule) {
...
int jmx_api_del_mac_filter_rule(cJSON *data_obj) {
	...
	if (mac_obj) {
		...
		u8 mac_bin[ETH_ALEN] = {0};
		if (mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
			return jmx_del_mac_from_rule(rule_id_obj->valueint, mac_bin);
		}
	} else {

		return jmx_del_mac_filter_rule(rule_id_obj->valueint);
	}
...
int jmx_match_mac_filter_whitelist(const unsigned char *mac) {
	...
	mac_filter_read_lock();
	node = jmx_find_mac_node(&g_mac_filter_whitelist, mac);
	ret = (node != NULL);
	mac_filter_read_unlock();
...
int jmx_api_add_mac_filter_whitelist(cJSON *data_obj) {
	...
	for (i = 0; i < cJSON_GetArraySize(mac_array); i++) {
		...
		if (mac_obj && mac_str_to_bin(mac_obj->valuestring, mac_bin)) {
			jmx_add_mac_node(&g_mac_filter_whitelist, mac_bin);
		}
	}
	mac_filter_write_unlock();
...
int jmx_api_del_mac_filter_whitelist(cJSON *data_obj) {
	...
	mac_filter_write_lock();
	node = jmx_find_mac_node(&g_mac_filter_whitelist, mac_bin);
	if (node) {
		hlist_del(&node->hlist);
		kfree(node);
		mac_filter_write_unlock();
