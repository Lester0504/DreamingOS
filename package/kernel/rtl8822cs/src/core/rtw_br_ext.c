...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove((((unsigned char *)ph + sizeof(struct pppoe_hdr)) + data_len), ((unsigned char *)ph + sizeof(struct pppoe_hdr)), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	memcpy(((unsigned char *)ph + sizeof(struct pppoe_hdr)), tag, data_len);
	return data_len;
}
				...
				if (ph->code == PADI_CODE || ph->code == PADR_CODE) {
					if (priv->ethBrExtInfo.addPPPoETag) {
						...
						if (pOldTag) { /* if SID existed, copy old value and delete it */
							...
							memcpy(((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN,
						...
						*pMagic = htons(MAGIC_CODE);
						memcpy(((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN, skb->data + ETH_ALEN, ETH_ALEN);
		...
		case NAT25_LOOKUP:
			if (ph->code == PADO_CODE || ph->code == PADS_CODE) {
				if (priv->ethBrExtInfo.addPPPoETag) {
					...
					memcpy(skb->data, ((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN, ETH_ALEN);
