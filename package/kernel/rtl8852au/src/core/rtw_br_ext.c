...
static __inline__ unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	start_ptr = cur_ptr = ((unsigned char *)ph + sizeof(struct pppoe_hdr));
...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	_rtw_memmove((((unsigned char *)ph + sizeof(struct pppoe_hdr)) + data_len), ((unsigned char *)ph + sizeof(struct pppoe_hdr)), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	_rtw_memcpy(((unsigned char *)ph + sizeof(struct pppoe_hdr)), tag, data_len);
	return data_len;
}
				...
				if (ph->code == PADI_CODE || ph->code == PADR_CODE) {
					if (priv->ethBrExtInfo.addPPPoETag) {
						...
						if (pOldTag) { /* if SID existed, copy old value and delete it */
							...
							_rtw_memcpy(((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN,
							       ((unsigned char *)pOldTag + sizeof(struct pppoe_tag)), old_tag_len);
						...
						pMagic = (unsigned short *)((unsigned char *)tag + sizeof(struct pppoe_tag));
						*pMagic = htons(MAGIC_CODE);
						_rtw_memcpy(((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN, skb->data + ETH_ALEN, ETH_ALEN);
		...
		case NAT25_LOOKUP:
			if (ph->code == PADO_CODE || ph->code == PADS_CODE) {
				if (priv->ethBrExtInfo.addPPPoETag) {
					...
					_rtw_memcpy(skb->data, ((unsigned char *)tag + sizeof(struct pppoe_tag)) + MAGIC_CODE_LEN, ETH_ALEN);
