...
第二个早期源码版本。相对 26.07.1 共 14 次增量提交，648 个文件变更。
...
- 12 个内部预览用 fixture：写死了本机开发运行时路径，只能在维护者机器上运行，
  连同仅用于读取它们的 3 个契约测试一并移除；
- `test_config_authority_contract.py` 与 `test_config_restore_boot_order_contract.py`：
  依赖未公开的 `dreamingwrt-installer`，在公开树中无法通过。

### 修正

- `system-users.js.gz` 此前为较新构建、而源文件仍是旧版，导致压缩资源与源码
  不一致，浏览器会取到源文件中不存在的内容（含本版不发布的 dreamingproxy
  权限项）。已按公开树源文件重新生成。
